...
# This section demonstrates the interface for calculating the spin evolution during a train of RF pulses, assuming balanced gradient moments. For this, we need the following packages:
...
# Now we have everything in place to calculate the signal. By default, the output is a complex valued array where each element describes the transversal magnetization ``x^f + i y^f`` of the free spin pool in each ``T_\text{R}``:
...
# As an alternative to the complex-valued signal, we can also calculate the full mangetization vector ``(x^f, y^f, z^f, x^s, z^s, 1)`` by supplying the keyword argument `output=:realmagnetization`. Here, ``x``, ``y``, ``z`` denote the dimensions in space, the superscript ``f`` and ``s`` denote the free and the semi-solid spin pool, respectively. We neglect the ``y^s`` component, assuming (without loss of generality) ωₓ = 0 and given that ``R_2^{s,l} \gg ω_0``.
...
# ## Gradients
# The same interface can also be used to calculate the derivatives of the signal wrt. the biophysical parameters. One can specify any number of derivatives with a vector of identifyer objects:
grad_list=[grad_m0s(), grad_R1f(), grad_R2f(), grad_Rx(), grad_R1s(), grad_T2s(), grad_ω0(), grad_B1()];

# One can specify any subset of the gradients in any order. Calling the function `calculatesignal_linearapprox` with this vector
s_linapp = calculatesignal_linearapprox(α, TRF, TR, ω0, B1, m0s, R1f, R2f, Rx, R1s, T2s, R2slT;
    grad_list=grad_list);
...
# ### Apparent R₁
# Above code calculates separate derivatives for ``R_1^f`` and ``R_1^s``. Yet, many publications, including our own work ["Rapid quantitative magnetization transfer imaging: utilizing the hybrid state and the generalized Bloch model"](http://TODO.org) assumes an apparent longitudinal relaxation rate ``R_1^a = R_1^f = R_1^f``. The derivatives wrt. this apparent relaxation rate can be calcuated with
