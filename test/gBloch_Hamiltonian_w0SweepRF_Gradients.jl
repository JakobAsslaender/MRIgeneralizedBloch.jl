...
_f = function (p)
    ...
    f_dφ(t) = f_φ(t) + ω0 * t
    T2s = 1e-5T2s
    sol = solve(DDEProblem(apply_hamiltonian_gbloch!, [m0[1], m0[2], m0[3], m0[4], M0], mfun, (0, TRF), (f_ω1, B1, f_dφ, m0s, R1f, R2f, Rex, R1s, T2s, G)), MethodOfSteps(Tsit5()))
    return reduce(hcat, [sol(t[i]) for i ∈ eachindex(t)])
end
_fd = jacobian(fd, _f, (M0, B1, 0, m0s, R1f, R2f, Rex, R1s, 1e5T2s))[1]
...
gBloch_sol_grad = solve(DDEProblem(apply_hamiltonian_gbloch!, [m0; zeros(5)], mfun_R1a, (0, TRF), (f_ω1, B1, f_φ, m0s, R1a, R2f, Rex, R1a, T2s, G, dGdT2s, grad_list)), MethodOfSteps(Tsit5()))
d_R1a = [gBloch_sol_grad(t[i])[j] for j ∈ 6:9, i ∈ eachindex(t)]

_f = function (R1a)
    sol = solve(DDEProblem(apply_hamiltonian_gbloch!, m0, mfun_R1a, (0, TRF), (f_ω1, B1, f_φ, m0s, R1a, R2f, Rex, R1a, T2s, G)), MethodOfSteps(Tsit5()))
    return reduce(hcat, [sol(t[i]) for i ∈ eachindex(t)])
end
_fd = fd(_f, R1a)
d_R1a_fd = _fd[1:4, :]
