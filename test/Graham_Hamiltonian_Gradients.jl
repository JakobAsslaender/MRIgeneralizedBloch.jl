...
Graham_sol_grad = solve(ODEProblem(apply_hamiltonian_graham_superlorentzian!, m0_grad, (0, TRF), (ω1, B1, ω0, TRF, m0s, R1f, R2f, Rex, R1s, T2s, grad_list)), ode_solver)

d_M0_ = [Graham_sol_grad(t[i])[j] for j ∈  6:9,  i ∈ eachindex(t)]
d_B1_ = [Graham_sol_grad(t[i])[j] for j ∈ 11:14, i ∈ eachindex(t)]
d_ω0_ = [Graham_sol_grad(t[i])[j] for j ∈ 16:19, i ∈ eachindex(t)]
d_m0s = [Graham_sol_grad(t[i])[j] for j ∈ 21:24, i ∈ eachindex(t)]
d_R1f = [Graham_sol_grad(t[i])[j] for j ∈ 26:29, i ∈ eachindex(t)]
d_R2f = [Graham_sol_grad(t[i])[j] for j ∈ 31:34, i ∈ eachindex(t)]
d_Rex = [Graham_sol_grad(t[i])[j] for j ∈ 36:39, i ∈ eachindex(t)]
d_R1s = [Graham_sol_grad(t[i])[j] for j ∈ 41:44, i ∈ eachindex(t)]
d_T2s = [Graham_sol_grad(t[i])[j] for j ∈ 46:49, i ∈ eachindex(t)]
...
_f = function (p)
    ...
    T2s = 1e-5T2s
    sol = solve(ODEProblem(apply_hamiltonian_graham_superlorentzian!, [m0[1], m0[2], m0[3], m0[4], M0], (0, TRF), (ω1, B1, ω0, TRF, m0s, R1f, R2f, Rex, R1s, T2s)), ode_solver)
...
@test d_ω0_ ≈ _fd[1:4, :, 3]          rtol = 1e-3
...
R1a = 1
grad_list = (grad_R1a(),)
Graham_sol_grad = solve(ODEProblem(apply_hamiltonian_graham_superlorentzian!, [m0; zeros(5)], (0, TRF), (ω1, B1, ω0, TRF, m0s, R1a, R2f, Rex, R1a, T2s, grad_list)), ode_solver)
