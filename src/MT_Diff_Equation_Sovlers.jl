...
function gBloch_calculate_magnetization_gradients(ω1, ω0::Float64, TR, TRF, m0s, R1, R2f, Rx, T2s, Niter)
    ...
    h30(p, t; idxs=nothing) = typeof(idxs) <: Number ? 0.0 : zeros(30)
    alg = MethodOfSteps(Tsit5())
    s = zeros(30, length(TRF))
    u0 = zeros(30)
    u0[3] = (1 - m0s)
    u0[4] = m0s
    u0[5] = 1.0
    ...
    # prep pulse 
    sol = solve(DDEProblem(gBloch_Hamiltonian_Gradient_ApproxFun!, u0, h30, (0.0, TRF[2]), (-ω1[2]/2, ω0, m0s, R1, R2f, T2s, Rx, ga, dg_oT2_a)), alg, save_everystep=false)  
    u0 = sol[end]
    
    T_FP = TR/2 - TRF[2]/2 - TRF[1]/2
    sol = solve(ODEProblem(FreePrecession_Hamiltonian_Gradient!, u0, (0.0, T_FP), (ω0, m0s, R1, R2f, Rx)), Tsit5(), save_everystep=false)
    u0 = sol[end]
    
    for ic = 0:(Niter - 1)
        for ip = 1:length(TRF)
            sol = solve(DDEProblem(gBloch_Hamiltonian_Gradient_ApproxFun!, u0, h30, (0.0, TRF[ip]), ((-1)^(ip + ic) * ω1[ip], ω0, m0s, R1, R2f, T2s, Rx, ga, dg_oT2_a)), alg, save_everystep=false)
            u0 = sol[end]
    
            local T_FP = TR - TRF[ip] / 2 - TRF[mod(ip, length(TRF)) + 1] / 2
            TE = TR / 2 - TRF[ip] / 2
            sol = solve(ODEProblem(FreePrecession_Hamiltonian_Gradient!, u0, (0.0, T_FP), (ω0, m0s, R1, R2f, Rx)), Tsit5(), save_everystep=false, saveat=TE)
            if sol.t[2] / TE - 1 > 1e-10
                throw(DimensionMismatch("sol.t[2] is not equal to TE"))
            end
            s[:,ip] = sol[2]
...
function gBloch_calculate_signal_gradients(ω1, ω0::Float64, TR,TRF, m0s, R1, R2f, Rx, T2s, Niter)
    s = gBloch_calculate_magnetization_gradients(ω1, ω0, TR, TRF, m0s, R1, R2f, Rx, T2s, Niter)
...
function Graham_calculate_magnetization_gradients(ω1, ω0::Float64, TR, TRF, m0s, R1, R2f, Rx, T2s, Niter)
    ...
    # prep pulse 
    sol = solve(ODEProblem(Graham_Hamiltonian_Gradient!, u0, (0.0, TRF[2]), (-ω1[2]/2, ω0, TRF[2], m0s, R1, R2f, T2s, Rx)), Tsit5(), save_everystep=false)    
    u0 = sol[end]
    ...
    for ic = 0:(Niter - 1)
        for ip = 1:length(TRF)
            sol = solve(ODEProblem(Graham_Hamiltonian_Gradient!, u0, (0.0, TRF[ip]), ((-1)^(ip + ic) * ω1[ip], ω0, TRF[ip], m0s, R1, R2f, T2s, Rx)), Tsit5(), save_everystep=false)
            u0 = sol[end]
...
function Graham_calculate_signal_gradients(ω1, ω0::Float64, TR, TRF, m0s, R1, R2f, Rx, T2s, Niter)
    s = Graham_calculate_magnetization_gradients(ω1, ω0, TR, TRF, m0s, R1, R2f, Rx, T2s, Niter)
    s = s[1:5:end,:] + 1im * s[2:5:end,:]
    return s
end
