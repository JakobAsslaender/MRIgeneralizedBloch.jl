...
function gBloch_calculate_magnetization(ω1, TRF, TR, ω0, B1, m0s, R1, R2f, Rx, T2s, grad_list, Niter)
    ...
    for ic = 0:(Niter - 1)
        ...
        sol = solve(DDEProblem(gBloch_Hamiltonian_ApproxFun!, u0, h, (0.0, TRF[1]), ((-1)^(1 + ic) * ω1[1], B1, ω0, m0s, R1, R2f, T2s, Rx, ga, dg_oT2_a, grad_list)), alg)
...
function LinearApprox_calculate_magnetization(ω1, TRF, TR, ω0, B1, m0s, R1, R2f, Rx, T2s, grad_list, Niter)
    ...
    Rrf(TRF, ω1, B1, T2s) = -log(fapprox([TRF/T2s, B1*ω1*T2s]))/TRF
    dRrfdT2s(TRF, ω1, B1, T2s) = (dfd1(TRF/T2s, B1 * ω1 * T2s) * TRF/T2s^2 - dfd2(TRF/T2s, B1 * ω1 * T2s) * B1 * ω1) / (TRF * fapprox(TRF/T2s, B1*ω1*T2s))
    dRrfdB1(TRF, ω1, B1, T2s) = - dfd2(TRF/T2s, B1 * ω1 * T2s) * ω1*T2s / (TRF * fapprox(TRF/T2s, B1*ω1*T2s))
    ...
    # prep pulse 
    sol = solve(ODEProblem(Linear_Hamiltonian!, u0, (0.0, TRF[2]), (-ω1[2]/2, B1, ω0, m0s, R1, R2f, Rx, (Rrf(TRF[2],ω1[2]/2,B1,T2s), dRrfdT2s(TRF[2],ω1[2]/2,B1,T2s), dRrfdB1(TRF[2],ω1[2]/2,B1,T2s)), grad_list)), Vern6(), save_everystep=false)
    u0 = sol[end]
    ...
    for ic = 0:(Niter - 1)
        ...
        u0 = sol[end]
        # calculate saturation of RF pulse
        sol = solve(ODEProblem(Linear_Hamiltonian!, u0, (0.0, TRF[1]), ((-1)^(1 + ic) * ω1[1], B1, ω0, m0s, R1, R2f, Rx, (Rrf(TRF[1], ω1[1], B1, T2s), dRrfdT2s(TRF[1], ω1[1], B1, T2s), dRrfdB1(TRF[1], ω1[1], B1, T2s)), grad_list)), Vern6(), save_everystep=false)
        ...
        for ip = 2:length(TRF)
            sol = solve(ODEProblem(Linear_Hamiltonian!, u0, (0.0, TRF[ip]), ((-1)^(ip + ic) * ω1[ip], B1, ω0, m0s, R1, R2f, Rx, (Rrf(TRF[ip], ω1[ip], B1, T2s), dRrfdT2s(TRF[ip], ω1[ip], B1, T2s), dRrfdB1(TRF[ip], ω1[ip], B1, T2s)), grad_list)), Vern6(), save_everystep=false)
            u0 = sol[end]
...
function LinearApprox_calculate_signal(ω1, TRF, TR, ω0, B1, m0s, R1, R2f, Rx, T2s, grad_list, Niter)
    s = LinearApprox_calculate_magnetization(ω1, TRF, TR, ω0, B1, m0s, R1, R2f, Rx, T2s, grad_list, Niter)
    s = s[1:5:end,:] + 1im * s[2:5:end,:]
    return s
end
