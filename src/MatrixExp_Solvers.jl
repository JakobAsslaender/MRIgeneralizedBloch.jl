...
function calculatesignal_linearapprox(α, TRF, TR, ω0, B1, m0s, R1f, R2f, Rx, R1s, R2s, dR2sdT2s, dR2sdB1; grad_list=(undef,), rfphase_increment=[π], m0=:periodic, preppulse=false, output=:complexsignal, isInversionPulse = [true; falses(length(α)-1)])
    ...
    for j in eachindex(grad_list), i in eachindex(rfphase_increment)
        if m0 == :periodic
            m = antiperiodic_boundary_conditions_linear(ω1, B1, ω0, TRF, TR, m0s, R1f, R2f, Rx, R1s, R2s, dR2sdT2s, dR2sdB1, grad_list[j], rfphase_increment[i], isInversionPulse)
        ...
        if preppulse
            k = (m0 == :IR) ? 2 : 1
            u_pr = exp(hamiltonian_linear(ω1[k] / 2, B1, ω0, TRF[k], m0s, R1f, R2f, Rx, R1s, R2s[k], dR2sdT2s[k], dR2sdB1[k], grad_list[j])) # R2sl is actually wrong for the prep pulse
            m = u_pr * m
        end

        Mij = @view S[:, i, jj[j]]
        propagate_magnetization_linear!(Mij, m, ω1, B1, ω0, TRF, TR, m0s, R1f, R2f, Rx, R1s, R2s, dR2sdT2s, dR2sdB1, grad_list[j], rfphase_increment[i], isInversionPulse)
    end
    return S
end
...
function antiperiodic_boundary_conditions_linear(ω1, B1, ω0, TRF, TR, m0s, R1f, R2f, Rx, R1s, R2s, dR2sdT2s, dR2sdB1, grad, rfphase_increment, isInversionPulse)
    A = evolution_matrix_linear(ω1, B1, ω0, TRF, TR, m0s, R1f, R2f, Rx, R1s, R2s, dR2sdT2s, dR2sdB1, grad, rfphase_increment, isInversionPulse)
...
function pulse_propagators(ω1, B1, ω0, TRF, TR, m0s, R1f, R2f, Rx, R1s, R2s, dR2sdT2s, dR2sdB1, grad, isInversionPulse)
