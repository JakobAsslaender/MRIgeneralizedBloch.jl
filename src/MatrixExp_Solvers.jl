...
############################################################################
"""
    calculatesignal_linearapprox(α, TRF, TR, ω0, B1, m0s, R1f, R2f, Rex, R1s, T2s, R2_mm_lT[; grad_list=(undef,), rfphase_increment=[π], m0=:periodic, output=:complexsignal, grad_moment = [i == 1 ? :spoiler_dual : :balanced for i ∈ eachindex(α)])
...
- `R2_mm_lT::NTuple{3, Function}`: Tuple of three functions: R2sl(TRF, ω1, B1, T2s), dR2sldB1(TRF, ω1, B1, T2s), and R2sldT2s(TRF, ω1, B1, T2s). Can be generated with [`precompute_R2sl`](@ref)
...
# Examples
```jldoctest
julia> R2_mm_lT = precompute_R2sl();

julia> calculatesignal_linearapprox(range(0, π/2, 100), fill(5e-4, 100), 4e-3, 0, 1, 0.1, 1, 15, 30, 6.5, 10e-6, R2_mm_lT)
...
```
"""
function calculatesignal_linearapprox(α, TRF, TR, ω0, B1, m0_rw, m0_mm, R1_fw, R1_rw, R1_mm, R2_fw, R2_rw, T2_mm, Rx_fw_mm, Rx_rw_fw, Rx_mm_rw, R2_mm_lT; grad_list=(undef,), rfphase_increment=[π], m0=:periodic, preppulse=false, output=:complexsignal, grad_moment = [i == 1 ? :spoiler_dual : :balanced for i ∈ eachindex(α)])

    R2_mm, dR2_mm_dT2_mm, dR2_mm_dB1 = evaluate_R2_mm_l_vector(abs.(α), TRF, B1, T2_mm, R2_mm_lT, grad_list)

    return calculatesignal_linearapprox(α, TRF, TR, ω0, B1, m0_rw, m0_mm, R1_fw, R1_rw, R1_mm, R2_fw, R2_rw, R2_mm, Rx_fw_mm, Rx_rw_fw, Rx_mm_rw, dR2_mm_dT2_mm, dR2_mm_dB1; grad_list, rfphase_increment, m0, preppulse, output, grad_moment)
end

function calculatesignal_linearapprox(α, TRF, TR, ω0, B1, m0_rw, m0_mm, R1_fw, R1_rw, R1_mm, R2_fw, R2_rw, R2_mm, Rx_fw_mm, Rx_rw_fw, Rx_mm_rw, dR2_mm_dT2_mm, dR2_mm_dB1; grad_list=(undef,), rfphase_increment=[π], m0=:periodic, preppulse=false, output=:complexsignal, grad_moment = [i == 1 ? :spoiler_dual : :balanced for i ∈ eachindex(α)])
    ...
    if m0 == :thermal || m0 == :IR
        if grad_list == (undef,)
            _m0 = @SVector [0, 0, 1 - m0s, 0, m0s, 1] # TODO
        else
            _m0 = @SVector [0, 0, 1 - m0s, 0, m0s, 0, 0, 0, 0, 0, 1] # TODO
        end
    elseif isa(m0, AbstractVector)
        if grad_list == (undef,)
            _m0 = length(m0) == 6 ? SVector{6}(m0) : SVector{6}([m0; zeros(5 - length(m0)); 1]) # TODO
        else
            _m0 = length(m0) == 11 ? SVector{11}(m0) : SVector{11}([m0; zeros(10 - length(m0)); 1]) # TODO
        end
    elseif m0 != :periodic
    ...
    for j in eachindex(grad_list), i in eachindex(rfphase_increment)
        if m0 == :periodic
            m = antiperiodic_boundary_conditions_linear(ω1, B1, ω0, TRF, TR, m0_rw, m0_mm, R1_fw, R1_rw, R1_mm, R2_fw, R2_rw, R2_mm, Rx_fw_mm, Rx_rw_fw, Rx_mm_rw, dR2_mm_dT2_mm, dR2_mm_dB1, grad_list[j], rfphase_increment[i], grad_moment)
        ...
        elseif m0 == :IR
            ...
            m = u_ip * _m0
            u_fp = exp(hamiltonian_linear(0, B1, ω0, TR, m0_rw, m0_mm, R1_fw, R1_rw, R1_mm, R2_fw, R2_rw, 0, Rx_fw_mm, Rx_rw_fw, Rx_mm_rw, 0, 0, grad_list[j]))
        ...
        if preppulse
            k = (m0 == :IR) ? 2 : 1
            u_pr = exp(hamiltonian_linear(ω1[k] / 2, B1, ω0, TRF[k], m0_rw, m0_mm, R1_fw, R1_rw, R1_mm, R2_fw, R2_rw, R2_mm[k], Rx_fw_mm, Rx_rw_fw, Rx_mm_rw, dR2_mm_dT2_mm[k], dR2_mm_dB1[k], grad_list[j])) # R2sl is actually wrong for the prep pulse
            m = u_pr * m
        end

        Mij = @view S[:, i, jj[j]]
        propagate_magnetization_linear!(Mij, m, ω1, B1, ω0, TRF, TR, m0_rw, m0_mm, R1_fw, R1_rw, R1_mm, R2_fw, R2_rw, R2_mm, Rx_fw_mm, Rx_rw_fw, Rx_mm_rw, dR2_mm_dT2_mm, dR2_mm_dB1, grad_list[j], rfphase_increment[i], grad_moment)
    end
    return S
end
...
# helper functions
############################################################################
function evolution_matrix_linear(ω1, B1, ω0, TRF, TR, m0_rw, m0_mm, R1_fw, R1_rw, R1_mm, R2_fw, R2_rw, R2_mm, Rx_fw_mm, Rx_rw_fw, Rx_mm_rw, dR2_mm_dT2_mm, dR2_mm_dB1, grad, rfphase_increment, grad_moment)
    ...
    u_fp, u_pl = pulse_propagators(ω1[1], B1, ω0, TRF[1], TR, m0_rw, m0_mm, R1_fw, R1_rw, R1_mm, R2_fw, R2_rw, R2_mm[1], Rx_fw_mm, Rx_rw_fw, Rx_mm_rw, dR2_mm_dT2_mm[1], dR2_mm_dB1[1], grad, grad_moment[1])
    A = u_fp * u_pl * u_rot * u_fp

    for i = length(ω1):-1:2
        u_fp, u_pl = pulse_propagators(ω1[i], B1, ω0, TRF[i], TR, m0_rw, m0_mm, R1_fw, R1_rw, R1_mm, R2_fw, R2_rw, R2_mm[i], Rx_fw_mm, Rx_rw_fw, Rx_mm_rw, dR2_mm_dT2_mm[i], dR2_mm_dB1[i], grad, grad_moment[i])
...
function antiperiodic_boundary_conditions_linear(ω1, B1, ω0, TRF, TR, m0_rw, m0_mm, R1_fw, R1_rw, R1_mm, R2_fw, R2_rw, R2_mm, Rx_fw_mm, Rx_rw_fw, Rx_mm_rw, dR2_mm_dT2_mm, dR2_mm_dB1, grad, rfphase_increment, grad_moment)
    A = evolution_matrix_linear(ω1, B1, ω0, TRF, TR, m0_rw, m0_mm, R1_fw, R1_rw, R1_mm, R2_fw, R2_rw, R2_mm, Rx_fw_mm, Rx_rw_fw, Rx_mm_rw, dR2_mm_dT2_mm, dR2_mm_dB1, grad, rfphase_increment, grad_moment)
...
function propagate_magnetization_linear!(S, m, ω1, B1, ω0, TRF, TR, m0_rw, m0_mm, R1_fw, R1_rw, R1_mm, R2_fw, R2_rw, R2_mm, Rx_fw_mm, Rx_rw_fw, Rx_mm_rw, dR2_mm_dT2_mm, dR2_mm_dB1, grad, rfphase_increment, grad_moment)
    u_rot = z_rotation_propagator(rfphase_increment, grad)
...
function pulse_propagators(ω1, B1, ω0, TRF, TR, m0_rw, m0_mm, R1_fw, R1_rw, R1_mm, R2_fw, R2_rw, R2_mm, Rx_fw_mm, Rx_rw_fw, Rx_mm_rw, dR2_mm_dT2_mm, dR2_mm_dB1, grad, grad_moment)
    if grad_moment == :balanced # balance all moments
        u_fp = exp(hamiltonian_linear(0, B1, ω0, (TR - TRF) / 2, m0_rw, m0_mm, R1_fw, R1_rw, R1_mm, R2_fw, R2_rw, 0, Rx_fw_mm, Rx_rw_fw, Rx_mm_rw, 0, 0, grad))
        u_fp = x_mm_destructor(u_fp) * u_fp
        u_pl = exp(hamiltonian_linear(ω1, B1, ω0, TRF, m0_rw, m0_mm, R1_fw, R1_rw, R1_mm, R2_fw, R2_rw, R2_mm, Rx_fw_mm, Rx_rw_fw, Rx_mm_rw, dR2_mm_dT2_mm, dR2_mm_dB1, grad))
    ...
    elseif grad_moment == :crusher # refocus the transverse magnetization that existed before the RF pulse, but crush the FID path of this pulse
        u_fp = exp(hamiltonian_linear(0, B1, ω0, TR / 2, m0_rw, m0_mm, R1_fw, R1_rw, R1_mm, R2_fw, R2_rw, 0, Rx_fw_mm, Rx_rw_fw, Rx_mm_rw, 0, 0, grad))
        u_fp = x_mm_destructor(u_fp) * u_fp
...
function ms_setindex!(S::AbstractArray{<:Complex}, y, i, grad::grad_param)
    S[i,1] = y[1] + 1im * y[2] + y[4] + 1im * y[5]
    S[i,2] = y[9] + 1im * y[10] + y[12] + 1im * y[13]
    return S
end