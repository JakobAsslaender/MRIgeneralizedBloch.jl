...
function apply_hamiltonian_gbloch!(∂m∂t, m, mfun, p::NTuple{11,Any}, t)
    ...
    if ω0 == 0
        xs = quadgk(x -> g((t - x) / T2s) * mfun(p, x; idxs=zs_idx), eps(), t, order=100)[1]
        ys = 0
    else
        xs = cos(ω0 * t) * quadgk(x -> cos(ω0 * x) * g((t - x) / T2s) * mfun(p, x; idxs=zs_idx), eps(), t, order=100)[1]
        ys = sin(ω0 * t) * quadgk(x -> sin(ω0 * x) * g((t - x) / T2s) * mfun(p, x; idxs=zs_idx), eps(), t, order=100)[1]
    end
...
function apply_hamiltonian_gbloch!(∂m∂t, m, mfun, p::Tuple{Function,Any,Number,Any,Any,Any,Any,Any,Any,Any,Any}, t)
    ...
    if ω0 == 0
        xs = quadgk(x -> ω1(x) * g((t - x) / T2s) * mfun(p, x; idxs=zs_idx), eps(), t, order=100)[1]
        ys = 0
    else
        xs = cos(ω0 * t) * quadgk(x -> ω1(x) * cos(ω0 * x) * g((t - x) / T2s) * mfun(p, x; idxs=zs_idx), eps(), t, order=100)[1]
        ys = sin(ω0 * t) * quadgk(x -> ω1(x) * sin(ω0 * x) * g((t - x) / T2s) * mfun(p, x; idxs=zs_idx), eps(), t, order=100)[1]
    end
...
function apply_hamiltonian_gbloch!(∂m∂t, m, mfun, p::Tuple{Function,Any,Function,Any,Any,Any,Any,Any,Any,Any,Any}, t)
    ...
    xs = cos(φ(t)) * quadgk(x -> ω1(x) * cos(φ(x)) * g((t - x) / T2s) * mfun(p, x; idxs=zs_idx), eps(), t, order=100)[1]
    ys = sin(φ(t)) * quadgk(x -> ω1(x) * sin(φ(x)) * g((t - x) / T2s) * mfun(p, x; idxs=zs_idx), eps(), t, order=100)[1]
...
function apply_hamiltonian_gbloch!(∂m∂t, m, mfun, p::NTuple{6,Any}, t)
    ...
    if ω0 == 0
        xs = quadgk(x -> g((t - x) / T2s) * mfun(p, x)[1], 0, t, order=100)[1]
        ys = 0
    else
        xs = cos(ω0 * t) * quadgk(x -> cos(ω0 * x) * g((t - x) / T2s) * mfun(p, x)[1], 0, t, order=100)[1]
        ys = sin(ω0 * t) * quadgk(x -> sin(ω0 * x) * g((t - x) / T2s) * mfun(p, x)[1], 0, t, order=100)[1]
    end
...
function apply_hamiltonian_gbloch!(∂m∂t, m, mfun, p::Tuple{Function,Any,Number,Any,Any,Any}, t)
    ...
    if ω0 == 0
        xs = quadgk(x -> ω1(x) * g((t - x) / T2s) * mfun(p, x)[1], 0, t, order=100)[1]
        ys = 0
    else
        xs = cos(ω0 * t) * quadgk(x -> ω1(x) * cos(ω0 * x) * g((t - x) / T2s) * mfun(p, x)[1], 0, t, order=100)[1]
        ys = sin(ω0 * t) * quadgk(x -> ω1(x) * sin(ω0 * x) * g((t - x) / T2s) * mfun(p, x)[1], 0, t, order=100)[1]
    end
...
function apply_hamiltonian_gbloch!(∂m∂t, m, mfun, p::Tuple{Function,Any,Function,Any,Any,Any}, t)
    ...
    xs = cos(φ(t)) * quadgk(x -> ω1(x) * cos(φ(x)) * g((t - x) / T2s) * mfun(p, x)[1], 0, t, order=100)[1]
    ys = sin(φ(t)) * quadgk(x -> ω1(x) * sin(φ(x)) * g((t - x) / T2s) * mfun(p, x)[1], 0, t, order=100)[1]
...
function add_partial_derivative!(∂m∂t, m, mfun, p::Tuple{Number,Any,Any,Any,Any,Any,Any,Any,Any,Function,Function}, t, grad_type::grad_T2s)
    ...
    xs = cos(ω0 * t) * quadgk(x -> cos(ω0 * x) * dG_o_dT2s_x_T2s((t - x) / T2s) * mfun(x), 0, t, order=100)[1]
    ys = sin(ω0 * t) * quadgk(x -> sin(ω0 * x) * dG_o_dT2s_x_T2s((t - x) / T2s) * mfun(x), 0, t, order=100)[1]
...
function add_partial_derivative!(∂m∂t, m, mfun, p::Tuple{Function,Any,Number,Any,Any,Any,Any,Any,Any,Function,Function}, t, grad_type::grad_T2s)
    ...
    xs = cos(ω0 * t) * quadgk(x -> ω1(x) * cos(ω0 * x) * dG_o_dT2s_x_T2s((t - x) / T2s) * mfun(x), 0, t, order=100)[1]
    ys = sin(ω0 * t) * quadgk(x -> ω1(x) * sin(ω0 * x) * dG_o_dT2s_x_T2s((t - x) / T2s) * mfun(x), 0, t, order=100)[1]
...
function add_partial_derivative!(∂m∂t, m, mfun, p::Tuple{Function,Any,Function,Any,Any,Any,Any,Any,Any,Function,Function}, t, grad_type::grad_T2s)
    ...
    xs = cos(φ(t)) * quadgk(x -> ω1(x) * cos(φ(x)) * dG_o_dT2s_x_T2s((t - x) / T2s) * mfun(x), 0, t, order=100)[1]
    ys = sin(φ(t)) * quadgk(x -> ω1(x) * sin(φ(x)) * dG_o_dT2s_x_T2s((t - x) / T2s) * mfun(x), 0, t, order=100)[1]
...
function add_partial_derivative!(∂m∂t, m, mfun, p::Tuple{Number,Any,Number,Any,Any,Any,Any,Any,Any,Function,Function}, t, grad_type::grad_ω0)
    ...
    xs = -sin(ω0 * t) * t * quadgk(x -> cos(ω0 * x)     * g((t - x) / T2s) * mfun(x), 0, t, order=100)[1]
    xs -= cos(ω0 * t)     * quadgk(x -> sin(ω0 * x) * x * g((t - x) / T2s) * mfun(x), 0, t, order=100)[1]

    ys  = cos(ω0 * t) * t * quadgk(x -> sin(ω0 * x)     * g((t - x) / T2s) * mfun(x), 0, t, order=100)[1]
    ys += sin(ω0 * t)     * quadgk(x -> cos(ω0 * x) * x * g((t - x) / T2s) * mfun(x), 0, t, order=100)[1]
...
function add_partial_derivative!(∂m∂t, m, mfun, p::Tuple{Function,Any,Number,Any,Any,Any,Any,Any,Any,Function,Function}, t, grad_type::grad_ω0)
    ...
    xs = -sin(ω0 * t) * t * quadgk(x -> ω1(x) * cos(ω0 * x)     * g((t - x) / T2s) * mfun(x), 0, t, order=100)[1]
    xs -= cos(ω0 * t)     * quadgk(x -> ω1(x) * sin(ω0 * x) * x * g((t - x) / T2s) * mfun(x), 0, t, order=100)[1]

    ys  = cos(ω0 * t) * t * quadgk(x -> ω1(x) * sin(ω0 * x)     * g((t - x) / T2s) * mfun(x), 0, t, order=100)[1]
    ys += sin(ω0 * t)     * quadgk(x -> ω1(x) * cos(ω0 * x) * x * g((t - x) / T2s) * mfun(x), 0, t, order=100)[1]
...
function add_partial_derivative!(∂m∂t, m, mfun, p::Tuple{Function,Any,Function,Any,Any,Any,Any,Any,Any,Function,Function}, t, grad_type::grad_ω0)
    ...
    ys  = cos(φ(t)) * t * quadgk(x -> ω1(x) * sin(φ(x))     * g((t - x) / T2s) * mfun(x), 0, t, order=100)[1]
    ys += sin(φ(t))     * quadgk(x -> ω1(x) * cos(φ(x)) * x * g((t - x) / T2s) * mfun(x), 0, t, order=100)[1]
...
function add_partial_derivative!(∂m∂t, m, mfun, p::Tuple{Number,Any,Number,Any,Any,Any,Any,Any,Any,Function,Any}, t, grad_type::grad_B1)
    ...
    xs = cos(ω0 * t) * quadgk(x -> cos(ω0 * x) * g((t - x) / T2s) * mfun(x), 0, t, order=100)[1]
    ys = sin(ω0 * t) * quadgk(x -> sin(ω0 * x) * g((t - x) / T2s) * mfun(x), 0, t, order=100)[1]
...
function add_partial_derivative!(∂m∂t, m, mfun, p::Tuple{Function,Any,Number,Any,Any,Any,Any,Any,Any,Function,Any}, t, grad_type::grad_B1)
    ...
    xs = cos(ω0 * t) * quadgk(x -> ω1(x) * cos(ω0 * x) * g((t - x) / T2s) * mfun(x), 0, t, order=100)[1]
    ys = sin(ω0 * t) * quadgk(x -> ω1(x) * sin(ω0 * x) * g((t - x) / T2s) * mfun(x), 0, t, order=100)[1]
...
function apply_hamiltonian_sled!(d∂m∂t, m, p::NTuple{6,Any}, t)
    ...
    xs = cos(ω0 * t) * quadgk(x -> g((t - x) / T2s) * cos(ω0 * x), 0, t, order=100)[1]
    ys = sin(ω0 * t) * quadgk(x -> g((t - x) / T2s) * sin(ω0 * x), 0, t, order=100)[1]
...
function apply_hamiltonian_sled!(∂m∂t, m, p::NTuple{10,Any}, t)
    ...
    if ω0 == 0
        xs = quadgk(x -> g((t - x) / T2s), 0, t, order=100)[1]
        ys = 0
    else
        xs = cos(ω0 * t) * quadgk(x -> g((t - x) / T2s) * cos(ω0 * x), 0, t, order=100)[1]
        ys = sin(ω0 * t) * quadgk(x -> g((t - x) / T2s) * sin(ω0 * x), 0, t, order=100)[1]
