...
u: 9-element Vector{Vector{Float64}}:
 ...
 [0.26002578672576243, 0.0, 0.7565529666157939, 0.18981913039644657, 1.0]
 [0.4610419882566735, 0.0, 0.6537242214798691, 0.16937688382096108, 1.0]
 [0.6661738538876188, 0.0, 0.44261236945975574, 0.1358931514238721, 1.0]
 [0.7923116826717913, 0.0, 0.1071314428045475, 0.09390268562369869, 1.0]
 [0.7994211188440823, 0.0, 0.00044033743550952185, 0.08214809683848684, 1.0]
...
function apply_hamiltonian_gbloch!(∂m∂t, m, mfun, p::NTuple{11,Any}, t)
    ...
    if ω0 == 0
        xs = quadgk(τ -> g((t - τ) / T2s) * mfun(p, τ; idxs=zs_idx), eps(), t, order=100)[1]
        ys = 0
    else
        xs = cos(ω0 * t) * quadgk(τ -> cos(ω0 * τ) * g((t - τ) / T2s) * mfun(p, τ; idxs=zs_idx), eps(), t, order=100)[1]
        ys = sin(ω0 * t) * quadgk(τ -> sin(ω0 * τ) * g((t - τ) / T2s) * mfun(p, τ; idxs=zs_idx), eps(), t, order=100)[1]
    end
...
function apply_hamiltonian_gbloch!(∂m∂t, m, mfun, p::Tuple{Function,Any,Number,Any,Any,Any,Any,Any,Any,Any,Any}, t)
    ...
    if ω0 == 0
        xs = quadgk(τ -> ω1(τ) * g((t - τ) / T2s) * mfun(p, τ; idxs=zs_idx), eps(), t, order=100)[1]
        ys = 0
    else
        xs = cos(ω0 * t) * quadgk(τ -> ω1(τ) * cos(ω0 * τ) * g((t - τ) / T2s) * mfun(p, τ; idxs=zs_idx), eps(), t, order=100)[1]
        ys = sin(ω0 * t) * quadgk(τ -> ω1(τ) * sin(ω0 * τ) * g((t - τ) / T2s) * mfun(p, τ; idxs=zs_idx), eps(), t, order=100)[1]
    end
...
function apply_hamiltonian_gbloch!(∂m∂t, m, mfun, p::Tuple{Function,Any,Function,Any,Any,Any,Any,Any,Any,Any,Any}, t)
    ...
    xs = cos(φ(t)) * quadgk(τ -> ω1(τ) * cos(φ(τ)) * g((t - τ) / T2s) * mfun(p, τ; idxs=zs_idx), eps(), t, order=100)[1]
    ys = sin(φ(t)) * quadgk(τ -> ω1(τ) * sin(φ(τ)) * g((t - τ) / T2s) * mfun(p, τ; idxs=zs_idx), eps(), t, order=100)[1]
...
function apply_hamiltonian_gbloch!(∂m∂t, m, mfun, p::NTuple{6,Any}, t)
    ...
    if ω0 == 0
        xs = quadgk(τ -> g((t - τ) / T2s) * mfun(p, τ)[1], 0, t, order=100)[1]
        ys = 0
    else
        xs = cos(ω0 * t) * quadgk(τ -> cos(ω0 * τ) * g((t - τ) / T2s) * mfun(p, τ)[1], 0, t, order=100)[1]
        ys = sin(ω0 * t) * quadgk(τ -> sin(ω0 * τ) * g((t - τ) / T2s) * mfun(p, τ)[1], 0, t, order=100)[1]
    end
...
function apply_hamiltonian_gbloch!(∂m∂t, m, mfun, p::Tuple{Function,Any,Number,Any,Any,Any}, t)
    ...
    if ω0 == 0
        xs = quadgk(τ -> ω1(τ) * g((t - τ) / T2s) * mfun(p, τ)[1], 0, t, order=100)[1]
        ys = 0
    else
        xs = cos(ω0 * t) * quadgk(τ -> ω1(τ) * cos(ω0 * τ) * g((t - τ) / T2s) * mfun(p, τ)[1], 0, t, order=100)[1]
        ys = sin(ω0 * t) * quadgk(τ -> ω1(τ) * sin(ω0 * τ) * g((t - τ) / T2s) * mfun(p, τ)[1], 0, t, order=100)[1]
    end
...
function apply_hamiltonian_gbloch!(∂m∂t, m, mfun, p::Tuple{Function,Any,Function,Any,Any,Any}, t)
    ...
    xs = cos(φ(t)) * quadgk(τ -> ω1(τ) * cos(φ(τ)) * g((t - τ) / T2s) * mfun(p, τ)[1], 0, t, order=100)[1]
    ys = sin(φ(t)) * quadgk(τ -> ω1(τ) * sin(φ(τ)) * g((t - τ) / T2s) * mfun(p, τ)[1], 0, t, order=100)[1]
...
function add_partial_derivative!(∂m∂t, m, mfun, p::Tuple{Number,Any,Any,Any,Any,Any,Any,Any,Any,Function,Function}, t, grad_type::grad_T2s)
    ...
    xs = cos(ω0 * t) * quadgk(τ -> cos(ω0 * τ) * dG_o_dT2s_x_T2s((t - τ) / T2s) * mfun(τ), 0, t, order=100)[1]
    ys = sin(ω0 * t) * quadgk(τ -> sin(ω0 * τ) * dG_o_dT2s_x_T2s((t - τ) / T2s) * mfun(τ), 0, t, order=100)[1]
...
function add_partial_derivative!(∂m∂t, m, mfun, p::Tuple{Function,Any,Number,Any,Any,Any,Any,Any,Any,Function,Function}, t, grad_type::grad_T2s)
    ...
    xs = cos(ω0 * t) * quadgk(τ -> ω1(τ) * cos(ω0 * τ) * dG_o_dT2s_x_T2s((t - τ) / T2s) * mfun(τ), 0, t, order=100)[1]
    ys = sin(ω0 * t) * quadgk(τ -> ω1(τ) * sin(ω0 * τ) * dG_o_dT2s_x_T2s((t - τ) / T2s) * mfun(τ), 0, t, order=100)[1]
...
function add_partial_derivative!(∂m∂t, m, mfun, p::Tuple{Function,Any,Function,Any,Any,Any,Any,Any,Any,Function,Function}, t, grad_type::grad_T2s)
    ...
    xs = cos(φ(t)) * quadgk(τ -> ω1(τ) * cos(φ(τ)) * dG_o_dT2s_x_T2s((t - τ) / T2s) * mfun(τ), 0, t, order=100)[1]
    ys = sin(φ(t)) * quadgk(τ -> ω1(τ) * sin(φ(τ)) * dG_o_dT2s_x_T2s((t - τ) / T2s) * mfun(τ), 0, t, order=100)[1]
...
function add_partial_derivative!(∂m∂t, m, mfun, p::Tuple{Number,Any,Number,Any,Any,Any,Any,Any,Any,Function,Function}, t, grad_type::grad_ω0)
    ...
    xs = -sin(ω0 * t) * t * quadgk(τ -> cos(ω0 * τ)     * g((t - τ) / T2s) * mfun(τ), 0, t, order=100)[1]
    xs -= cos(ω0 * t)     * quadgk(τ -> sin(ω0 * τ) * τ * g((t - τ) / T2s) * mfun(τ), 0, t, order=100)[1]

    ys  = cos(ω0 * t) * t * quadgk(τ -> sin(ω0 * τ)     * g((t - τ) / T2s) * mfun(τ), 0, t, order=100)[1]
    ys += sin(ω0 * t)     * quadgk(τ -> cos(ω0 * τ) * τ * g((t - τ) / T2s) * mfun(τ), 0, t, order=100)[1]
...
function add_partial_derivative!(∂m∂t, m, mfun, p::Tuple{Function,Any,Number,Any,Any,Any,Any,Any,Any,Function,Function}, t, grad_type::grad_ω0)
    ...
    xs = -sin(ω0 * t) * t * quadgk(τ -> ω1(τ) * cos(ω0 * τ)     * g((t - τ) / T2s) * mfun(τ), 0, t, order=100)[1]
    xs -= cos(ω0 * t)     * quadgk(τ -> ω1(τ) * sin(ω0 * τ) * τ * g((t - τ) / T2s) * mfun(τ), 0, t, order=100)[1]

    ys  = cos(ω0 * t) * t * quadgk(τ -> ω1(τ) * sin(ω0 * τ)     * g((t - τ) / T2s) * mfun(τ), 0, t, order=100)[1]
    ys += sin(ω0 * t)     * quadgk(τ -> ω1(τ) * cos(ω0 * τ) * τ * g((t - τ) / T2s) * mfun(τ), 0, t, order=100)[1]
...
function add_partial_derivative!(∂m∂t, m, mfun, p::Tuple{Function,Any,Function,Any,Any,Any,Any,Any,Any,Function,Function}, t, grad_type::grad_ω0)
    ...
    xs = -sin(φ(t)) * t * quadgk(τ -> ω1(τ) * cos(φ(τ))     * g((t - τ) / T2s) * mfun(τ), 0, t, order=100)[1]
    xs -= cos(φ(t))     * quadgk(τ -> ω1(τ) * sin(φ(τ)) * τ * g((t - τ) / T2s) * mfun(τ), 0, t, order=100)[1]

    ys  = cos(φ(t)) * t * quadgk(τ -> ω1(τ) * sin(φ(τ))     * g((t - τ) / T2s) * mfun(τ), 0, t, order=100)[1]
    ys += sin(φ(t))     * quadgk(τ -> ω1(τ) * cos(φ(τ)) * τ * g((t - τ) / T2s) * mfun(τ), 0, t, order=100)[1]
...
function add_partial_derivative!(∂m∂t, m, mfun, p::Tuple{Number,Any,Number,Any,Any,Any,Any,Any,Any,Function,Any}, t, grad_type::grad_B1)
    ...
    xs = cos(ω0 * t) * quadgk(τ -> cos(ω0 * τ) * g((t - τ) / T2s) * mfun(τ), 0, t, order=100)[1]
    ys = sin(ω0 * t) * quadgk(τ -> sin(ω0 * τ) * g((t - τ) / T2s) * mfun(τ), 0, t, order=100)[1]
...
function add_partial_derivative!(∂m∂t, m, mfun, p::Tuple{Function,Any,Number,Any,Any,Any,Any,Any,Any,Function,Any}, t, grad_type::grad_B1)
    ...
    xs = cos(ω0 * t) * quadgk(τ -> ω1(τ) * cos(ω0 * τ) * g((t - τ) / T2s) * mfun(τ), 0, t, order=100)[1]
    ys = sin(ω0 * t) * quadgk(τ -> ω1(τ) * sin(ω0 * τ) * g((t - τ) / T2s) * mfun(τ), 0, t, order=100)[1]
...
function add_partial_derivative!(∂m∂t, m, mfun, p::Tuple{Function,Any,Function,Any,Any,Any,Any,Any,Any,Function,Any}, t, grad_type::grad_B1)
    ...
    xs = cos(φ(t)) * quadgk(τ -> ω1(τ) * cos(φ(τ)) * g((t - τ) / T2s) * mfun(τ), 0, t, order=100)[1]
    ys = sin(φ(t)) * quadgk(τ -> ω1(τ) * sin(φ(τ)) * g((t - τ) / T2s) * mfun(τ), 0, t, order=100)[1]
...
function apply_hamiltonian_sled!(d∂m∂t, m, p::NTuple{6,Any}, t)
    ...
    xs = cos(ω0 * t) * quadgk(τ -> g((t - τ) / T2s) * cos(ω0 * τ), 0, t, order=100)[1]
    ys = sin(ω0 * t) * quadgk(τ -> g((t - τ) / T2s) * sin(ω0 * τ), 0, t, order=100)[1]
...
function apply_hamiltonian_sled!(∂m∂t, m, p::NTuple{10,Any}, t)
    ...
    if ω0 == 0
        xs = quadgk(τ -> g((t - τ) / T2s), 0, t, order=100)[1]
        ys = 0
    else
        xs = cos(ω0 * t) * quadgk(τ -> g((t - τ) / T2s) * cos(ω0 * τ), 0, t, order=100)[1]
        ys = sin(ω0 * t) * quadgk(τ -> g((t - τ) / T2s) * sin(ω0 * τ), 0, t, order=100)[1]
